...
SpawnEkle(playerid)
{
	...
	new query[150];
	mysql_format(exConnect, query, sizeof(query), "INSERT INTO `spawnlar` (`ID`, `SpawnX`, `SpawnY`, `SpawnZ`, `SpawnA`) VALUES ('%d', '%f', '%f', '%f', '%f')", i, SpawnData[i][spawnX], SpawnData[i][spawnY], SpawnData[i][spawnZ], SpawnData[i][spawnA]);
	mysql_tquery(exConnect, query);

	SendClientMessage(playerid,-1,"Spawn eklendi.");
	return 1;
}
...
SpawnSil(spawnid)
{
	...
	SpawnData[spawnid][spawnA] = 0.0;
	DestroyDynamicPickup(SpawnData[spawnid][spawnPickup]);
	DestroyDynamic3DTextLabel(SpawnData[spawnid][spawn3D]);
	DestroyDynamicMapIcon(SpawnData[spawnid][spawnIcon]);
	Iter_Remove(SpawnIter, spawnid);

	new query[64];
	mysql_format(exConnect, query, sizeof(query), "DELETE FROM `spawnlar` WHERE `ID` = %d", spawnid);
	mysql_tquery(exConnect, query);
	return 1;
}
	...
	if(!rows)
	{
		...
		new x, loaded;
 		while(loaded < rows) {
			...
			cache_get_value_name_float(loaded, "SpawnX", SpawnData[x][spawnX]);
			cache_get_value_name_float(loaded, "SpawnY", SpawnData[x][spawnY]);
			cache_get_value_name_float(loaded, "SpawnZ", SpawnData[x][spawnZ]);
			cache_get_value_name_float(loaded, "SpawnA", SpawnData[x][spawnA]);
...
{
	exConnect = mysql_connect(MYSQL_HOST, MYSQL_USER, MYSQL_PASSWORD, MYSQL_DATABASE);
	...
	mysql_tquery(exConnect, "CREATE TABLE IF NOT EXISTS `spawnlar` (`ID` INT(11) NOT NULL, `SpawnX` VARCHAR(16) NOT NULL, `SpawnY` VARCHAR(16) NOT NULL, `SpawnZ` VARCHAR(16) NOT NULL, `SpawnA` VARCHAR(16) NOT NULL, PRIMARY KEY (`ID`))");
	mysql_tquery(exConnect, "SELECT * FROM `spawnlar`", "SpawnYukle");
	return 1;
}
public OnPlayerSpawn(playerid)
{
	new spawnID = Iter_Random(SpawnIter);
	SetPlayerPos(playerid, SpawnData[spawnID][spawnX], SpawnData[spawnID][spawnY], SpawnData[spawnID][spawnZ]);
	SetPlayerFacingAngle(playerid, SpawnData[spawnID][spawnA]);
	return 1;
}
